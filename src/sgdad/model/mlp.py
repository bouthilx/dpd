...
class MLP(nn.Module):
    def __init__(self, input_size=28*28, num_classes=10, layers=tuple()):
        ...
        for i, [insize, outsize] in enumerate(zip(insizes, outsizes)):
            setattr(self, 'fc{}'.format(i), nn.Linear(insize, outsize))
...
def build(input_size, num_classes, layers):
    if isinstance(input_size, list):
        input_size = numpy.product(input_size)

    return MLP(input_size=input_size, num_classes=num_classes, layers=layers)
