...
class WorkerAllocationHistory:
    ...
    def generate_gantt_entry(self, trial):
    ...
    def generate_gantt(self):
        return [self.generate_gantt_entry(trial) for trial in self.trials]
    ...
    for tindex, trial in enumerate(trials):
        ...
        for (ttype, date) in timestamps:
            ...
            if ttype == 'start':
                ...
                work_segments[segment_start] = TrialWork(
                    trial_id=tindex,
                    start=segment_start,
                    end=time,
...
def make_gantt(workers_allocs):
    ...
    fig = ff.create_gantt(gantt_entries, index_col='Complete', group_tasks=True, show_colorbar=True)
...
if __name__ == '__main__':
    t = read_report('tests/output_sample.json')
    visualize_problem('ef90ce916efb051', t)
