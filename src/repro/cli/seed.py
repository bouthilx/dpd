...
from repro.seed import run, create_trial
...
def main(argv=None):
    ...
    parser.add_argument(
        '--container', help='Container to execute trials')
    parser.add_argument(
    ...
    for dataset_name in options.datasets:
        for model_name in options.models:
            ...
            if any(True for _ in mahler_client.find(tags=tags + [run.name])) and not options.force:
                print('`seed` already registered for tags: {}'.format(", ".join(tags)))
                continue

            print("Registering {} with tags: {}".format(run.name, ", ".join(tags)))
            ...
            create_trial(
                ...
                tags=tags, container=options.container)
