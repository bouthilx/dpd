...
--tags analysis;{experiment};{dataset};{model};{analysis_version};epoch-{epoch}"""
...
def fetch_trial_ids(database, dataset, model, version):
    tags = [EXPERIMENT, dataset, model, version, 'execution']
    query = {
        'tags': {'$all': tags},
        ...
        for trial_id in fetch_trial_ids(database, dataset, model, args.execution_version):
            for epoch in epochs:
                kleio = kleio_template.format(
                    ...
                    analysis_version=args.analysis_version, epoch=epoch)
